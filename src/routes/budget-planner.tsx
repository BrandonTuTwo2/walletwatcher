                ...
                <Table.Cell>
                  <a
                    ...
                    onClick={(e) => {
                      ...
                      const eleToRemove = tempy[eleIndex]
                      console.log(eleToRemove)
                      tempy.splice(eleIndex, 1)
                      setTableDisplay(tempy);
                      if (eleToRemove.price) {
                        if (eleToRemove.type === "Food") {
                          setFoodCost(foodCost - eleToRemove.price)
                          //let tempBudgetleft = budget + eleToRemove.price;
                        } else if (eleToRemove.type === "Entertainment") {
                          setEntCost(foodCost - eleToRemove.price)
                        } else if (eleToRemove.type === "Transportation") {
                          setTransCost(foodCost - eleToRemove.price)
                        } else if (eleToRemove.type === "Emergency") {
                          setEmergCost(foodCost - eleToRemove.price)
                        }

                        setBudget(budget + eleToRemove.price)
                      }

                      //For some reason the table doesn't automatically remove the item so I have to set the display to none
                      if (element.id) {
